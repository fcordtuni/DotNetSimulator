...
internal static class DataSeriesFilter
{
		...
		DateTime endDateTime, TimeSpan samplingInterval, TimeSpan smoothingWindow)
	{
		...
		if (duration.Ticks < 0) throw new ArgumentException("Ticks < 0");

		var points = new (double Value, double Count)[checked((int)Math.Max(1, duration.Ticks / samplingInterval.Ticks))];

		var halfWindowTicks = Math.Max(1, smoothingWindow.Ticks >> 1);

		var firstPointIndex = 0;
		var firstPointDateTime = startDateTime;
